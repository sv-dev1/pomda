...
if ( $title ) {
	$output .= '<header class="card__header">';
		...
		if ( $show_all_staff_link ) {
			$output .= '<a class="sp-staff-directory-link sp-view-all-link btn btn-default btn-outline btn-xs card-header__button" href="' . get_permalink( $id ) . '">' . esc_html__( 'View all staff', 'alchemists' ) . '</a>';
		}
	$output .= '</header>';
}
