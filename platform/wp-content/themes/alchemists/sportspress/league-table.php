...
if ( $title ) {
	$output .= '<header class="card__header">';
		...
		if ( $show_full_table_link ) {
			$output .= '<a href="' . get_permalink( $id ) . '" class="btn btn-default btn-outline btn-xs card-header__button">' . esc_html__( 'View full table', 'alchemists' ) . '</a>';
		}
	$output .= '</header>';
}
...
$output .= '<th class="data-rank">' . esc_html__( 'Pos', 'alchemists' ) . '</th>';
