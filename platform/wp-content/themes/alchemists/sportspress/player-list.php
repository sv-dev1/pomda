	...
	if ( $title ) {
		$output .= '<header class="card__header">';
			...
			if ( $show_all_players_link ):
				$output .= '<a href="' . get_permalink( $id ) . '" class="btn btn-default btn-outline btn-xs card-header__button">' . esc_html__( 'View all players', 'alchemists' ) . '</a>';
			endif;
...
foreach ( $groups as $group ):
	...
	$output .= '<div class="table-responsive sp-table-wrapper">' .
		'<table class="table table--lg team-roster-table sp-player-list sp-data-table' . ( $sortable ? ' sp-sortable-table' : '' ). ( $responsive ? ' sp-responsive-table ' . $identifier : '' ) . ( $scrollable ? ' sp-scrollable-table' : '' ) . ( $paginated ? ' sp-paginated-table' : '' ) . '" data-sp-rows="' . $rows . '">' . '<thead>' . '<tr>';

	if ( ! is_array( $labels ) || array_key_exists( 'number', $labels ) ):
		if ( in_array( $orderby, array( 'number', 'name' ) ) ):
			$output .= '<th class="data-number">#</th>';
		else:
			$output .= '<th class="data-rank">' . esc_html__( 'Rank', 'alchemists' ) . '</th>';
		endif;
	endif;
