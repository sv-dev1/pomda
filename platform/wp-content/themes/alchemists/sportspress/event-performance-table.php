							...
							foreach ( $data as $player_id => $row ):
								...
								if ( $stars_type ):
									...
									if ( $player_stars ):
										switch ( $stars_type ):
											case 1:
												$name .= ' <span class="sp-event-stars"><i class="sp-event-star dashicons dashicons-star-filled" title="' . esc_html__( 'Player of the Match', 'alchemists' ) . '"></i></span>';
												break;
											case 2:
												$name .= ' <span class="sp-event-stars">' . str_repeat( '<i class="sp-event-star dashicons dashicons-star-filled" title="' . esc_html__( 'Stars', 'alchemists' ) . '"></i>', $player_stars ) . '</span>';
												break;
											case 3:
												$name .= ' <span class="sp-event-stars"><i class="sp-event-star sp-event-star-' . $player_stars . '  dashicons dashicons-star-filled" title="' . esc_html__( 'Stars', 'alchemists' ) . '"></i><span class="sp-event-star-number">' . $player_stars . '</span></span>';
												break;
										endswitch;
									endif;
								endif;
								...
								echo '<td class="data-name" data-label="' . ( isset( $section_label ) ? $section_label : esc_html__( 'Player', 'alchemists' ) ) .'">' . $name . '</td>';
							...
							if ( $show_total && ( ! $primary || sizeof( array_intersect_key( $totals, array_flip( (array) $primary ) ) ) ) ) {
								...
								<tr class="sp-total-row <?php echo ( $i % 2 == 0 ? 'odd' : 'even' ); ?>">
									...
									if ( $show_players ):
										...
										if ( $mode == 'values' ):
											echo '<td class="data-name" data-label="&nbsp;">' . esc_html__( 'Total', 'alchemists' ) . '</td>';
										endif;
									endif;
