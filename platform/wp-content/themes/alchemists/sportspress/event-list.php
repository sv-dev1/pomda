			...
			<table class="table table-hover team-schedule team-schedule--full sp-event-list sp-event-list-format-<?php echo esc_attr( $title_format ); ?> sp-data-table<?php if ( $paginated ) { ?> sp-paginated-table<?php } if ( $sortable ) { ?> sp-sortable-table<?php } if ( $responsive ) { echo ' sp-responsive-table '.$identifier; } if ( $scrollable ) { ?> sp-scrollable-table <?php } ?>" data-sp-rows="<?php echo $rows; ?>">
				<thead>
					<tr>
						...
						if ( sp_column_active( $usecolumns, 'league' ) )
							echo '<th class="data-league">' . esc_html__( 'Competition', 'alchemists' ) . '</th>';
						...
						if ( sp_column_active( $usecolumns, 'venue' ) )
							echo '<th class="data-venue">' . esc_html__( 'Venue', 'alchemists' ) . '</th>';
				...
				<tbody>
					...
					foreach ( $data as $event ):
						...
						echo '<td class="data-date" itemprop="startDate" content="' . mysql2date( 'Y-m-d\TH:iP', $event->post_date ) . '" data-label="' . esc_attr__( 'Date', 'alchemists' ) . '">' . $date_html . '</td>';

							switch ( $title_format ) {
								case 'homeaway':
									if ( sp_column_active( $usecolumns, 'event' ) ) {
										$team = array_shift( $teams_array );
										echo '<td class="data-home' . $team_class . '" itemprop="competitor" itemscope itemtype="http://schema.org/SportsTeam" data-label="' . esc_attr__( 'Home', 'alchemists' ) . '">' . $team . '</td>';
									}

									if ( 'combined' == $time_format && sp_column_active( $usecolumns, 'time' ) ) {
										echo '<td class="data-time" data-label="' . esc_html__( 'Time/Results', 'alchemists' ) . '">';
										...
										echo '<td class="data-results" data-label="' . esc_html__( 'Results', 'alchemists' ) . '">';
										if ( $link_events ) echo '<a href="' . get_post_permalink( $event->ID, false, true ) . '" itemprop="url">';
										if ( ! empty( $main_results ) ):
											echo implode( ' - ', $main_results );
										else:
									...
									if ( sp_column_active( $usecolumns, 'event' ) ) {
										$team = array_shift( $teams_array );
										echo '<td class="data-away' . $team_class . '" itemprop="competitor" itemscope itemtype="http://schema.org/SportsTeam" data-label="' . esc_attr__( 'Away', 'alchemists' ) . '">' . $team . '</td>';
									}

									if ( in_array( $time_format, array( 'separate', 'time' ) ) && sp_column_active( $usecolumns, 'time' ) ) {
										echo '<td class="data-time" data-label="' . esc_attr__( 'Time', 'alchemists' ) . '">';
										if ( $link_events ) echo '<a href="' . get_post_permalink( $event->ID, false, true ) . '" itemprop="url">';
										echo '<date>&nbsp;' . get_post_time( 'H:i:s', false, $event ) . '</date>' . apply_filters( 'sportspress_event_time', sp_get_time( $event ), $event->ID );
										if ( $link_events ) echo '</a>';
										echo '</td>';
								...
								default:
									if ( sp_column_active( $usecolumns, 'event' ) ) {
										if ( $title_format == 'teams' ) {
											echo '<td class="data-event data-teams" data-label="' . esc_attr__( 'Teams', 'alchemists' ) . '">' . $teams_output . '</td>';
										} else {
											...
											echo '<td class="data-event" data-label="' . esc_attr__( 'Event', 'alchemists' ) . '">' . $title_html . '</td>';
										}
									}
									...
									switch ( $time_format ) {
										...
										default:
											if ( sp_column_active( $usecolumns, 'time' ) ) {
												echo '<td class="data-time" data-label="' . esc_attr__( 'Time/Results', 'alchemists' ) . '">';
												if ( $link_events ) echo '<a href="' . get_post_permalink( $event->ID, false, true ) . '" itemprop="url">';
												if ( ! empty( $main_results ) ):
													echo implode( ' - ', $main_results );
												else:
							...
							if ( sp_column_active( $usecolumns, 'venue' ) ):
								echo '<td class="data-venue" data-label="' . esc_attr__( 'Venue', 'alchemists' ) . '">';
								if ( $link_venues ):
