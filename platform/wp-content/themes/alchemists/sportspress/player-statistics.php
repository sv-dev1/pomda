...
$leagues = wp_get_post_terms( $id, 'sp_league', array(
	'order'   => 'DESC',
...
if ( 1 == $sections ) {
	$section_order = array( 1 => esc_html__( 'Defense', 'alchemists' ), 0 => esc_html__( 'Offense', 'alchemists' ) );
} elseif ( 0 == $sections ) {
	$section_order = array( esc_html__( 'Offense', 'alchemists' ), esc_html__( 'Defense', 'alchemists' ) );
} else {
	...
	foreach ( $section_order as $section_id => $section_label ) {
		...
		if ( $show_career_totals ) {
			sp_get_template( 'player-statistics-league.php', array(
				...
				'caption' => esc_html__( 'Career Total', 'alchemists' ),
				'scrollable' => $scrollable,
				'hide_teams' => true,
			) );
		}
