...
class Alchemists_Widget_Instagram_Feed extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		// Set User Name from Theme Options > Social > Instagram if the widget field is empty
		if ( empty( $user_name ) ) {
			$user_name = $alchemists_insta_user;
		}

		// Set User Id from Theme Options > Social > Instagram if the widget field is empty
		if ( empty( $user_id ) ) {
			$user_id_arr = explode( '.', $alchemists_insta_token, 2 );
			$user_id = $user_id_arr[0];
		}

		// Set Access Token from Theme Options > Social > Instagram if the widget field is empty
		if ( empty( $access_token ) ) {
			$access_token = $alchemists_insta_token;
		}
		?>
	...
	function form( $instance ) {
		...
		<p class="alchemists-widget-note">
			* <?php esc_html_e( 'If empty will be taken from Theme Options > Social > Instagram', 'alchemists' ); ?>
		</p>
