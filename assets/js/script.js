...
(function($){
	...
	var currenttimezone = $('.token-countdown').attr("data-zone");
	var countdown = ($('.token-countdown').attr("data-date"));
	var timefun = function (countdown,currenttimezone){
    var date = moment.tz(countdown,"YYYY/MM/DD",currenttimezone);
	...
	if ($count_token.length > 0 ) {
		$count_token.each(function() {
			...
			$self.countdown(timefun(datetime,currenttimezone)).on('update.countdown', function(event) {
